...
- Search your passwords via the keyword `pw`.
    ...
    - <kbd>⌘</kbd><kbd>⏎</kbd>: Edit entry in your Terminal, using
      [the Terminal configured in your Alfred settings](https://www.alfredapp.com/help/features/terminal/)
      and your `$EDITOR`.
...
- If there is an issue, open the Alfred debugger via `cmd+d` in the workflow
  window and see what is reported there.
- The error `gpg: public key decryption failed: No pinentry` sometimes arises
  when updating `gpg` keys or `gpg` itself. This is usually fixed by restarting
  `gpg` via `gpgconf --kill gpg-agent`.
